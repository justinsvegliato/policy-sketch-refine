...
class AbstractMDP:
    ...
    def __check_uniformity(self, mdp, abstract_states):
        for abstract_state in abstract_states:
    ...
    def __compute_abstract_states(self, mdp):
        ...
        is_uniform, abstract_state = self.__check_uniformity(mdp, abstract_states)
        ...
        while L:
            ...
            for every block C in current partition and every action a:
                Cprime = C intersect #a^-1(B) (the set of all states that have some probability of transitioning to B)
                if Cprime not empty and cprime != C:
                    replace block C in partition with Cprime and (C setminus Cprime)
